...
plot_kmean_block(full_img, 3, 3)
```

```{r}
create_train_val_test = function(full_img){
  image1_kmean <- kmeans(full_img %>% filter(image == 1) %>% select(Y,X) ,4)
  image2_kmean <- kmeans(full_img %>% filter(image == 2) %>% select(Y,X) ,4)
  image3_kmean <- kmeans(full_img %>% filter(image == 3) %>% select(Y,X) ,4)
  cluster_num <- c(image1_kmean$cluster, image2_kmean$cluster + 4, image3_kmean$cluster + 8)
  full_img <- full_img %>% mutate(cluster_num = cluster_num)
  sets <- split(sample(1:12), rep(1:3, c(2,2,8)))
  training <- full_img %>% filter(cluster_num %in% sets$`3`)
  valid <- full_img %>% filter(cluster_num %in% sets$`2`)
  test <- full_img %>% filter(cluster_num %in% sets$`1`)
  
  return(list(train = training, validation = valid, test = test))
}
full_split = create_train_val_test(full_img)

```

## Baseline
```{r}
valid_accuracy <- mean((full_split$validation %>% filter(Label != 0) %>% pull(Label) )== -1)
test_accuracy <- mean((full_split$test %>% filter(Label != 0) %>% pull(Label) )== -1)
```
