...
for (i in 1:3){
  ...
  scale_fill_manual(values=c("Cloudy (1)"='white', 
                             'Cloud-free (-1)'='grey',
                             'Unlabeled (0)'='black'))+
  labs(title=paste0('Image ',i,' Label'), x='X-coordinate', y='Y-coordinate')+
...
plot_block = function(data, split_func, K, img){
...
plot_block(full_img,block_split, 3, 1)
...
plot_block(full_img,Kmeans_block_split, 3, 1)
```
...
baseline_accuracy = function(bk_split, kmeans_split){
  ...
  print(paste0('K-means Test Baseline accuracy is: ', kmeans_test_accuracy))
  print(paste0('Block Test Baseline accuracy is: ', block_test_accuracy))
  
  print(paste0('K-means Valid Baseline accuracy is: ', kmeans_valid_accuracy))
  print(paste0('Block Valid Baseline accuracy is: ', block_valid_accuracy))
}
...
foi$coeffs %>% as.data.frame() %>% mutate(Variable=rownames(.)) %>% pivot_longer(1:2, names_to = 'split', values_to = 'coef') %>%
  dplyr::select(-contains('Avg'))%>%
...
## CVmaster
```{r}
# excld_cols are columns that are not features, but required for block split / k-means split
...
xgboost_model = boost_tree(tree_depth = tune(), trees = 300, learn_rate = tune(), min_n = tune(), loss_reduction = tune(), sample_size = tune(), stop_iter = tune())%>%set_engine('xgboost', num.threads=12) %>%set_mode('classification')



## ---------------------------- K-means split -------------------------------------
train_data = rbind(kmeans_split$train, kmeans_split$val)
...
train_data2 = rbind(bk_split$train, bk_split$val)
...
```{r}
roc_train_data <- train_data %>% select(-c(Y,X, image)) %>% mutate(across(Label, ~ if_else(.x == -1, 0, 1))) %>% mutate(across(Label, factor))
log_recipe <- recipe(Label ~ . , data = roc_train_data) %>% step_normalize(all_numeric_predictors())
log_model <- logistic_reg() %>% set_engine('glm')
log_res <- workflow() %>% add_recipe(log_recipe) %>% add_model(log_model) %>% fit(roc_train_data)
log_roc <- log_res %>% augment(new_data = roc_train_data) %>% roc_curve(Label, .pred_1)

log_res %>% augment(new_data = roc_train_data)
log_roc %>% ggplot() + geom_point(aes(1-specificity, sensitivity)) + labs(title = "AUC = 0.9533")
log_auc <- log_res %>% augment(new_data = roc_train_data) %>% roc_auc(Label, .pred_0)
log_auc

log_reg_recipe <- recipe(Label ~ . , data = roc_train_data) %>% step_normalize(all_numeric_predictors())
log_reg_model <- logistic_reg(mixture = 0.829, penalty = 0.0279) %>% set_engine('glmnet')
log_reg_res <- workflow() %>% add_recipe(log_reg_recipe) %>% add_model(log_reg_model) %>% fit(roc_train_data)
log_reg_roc <- log_reg_res %>% augment(new_data = roc_train_data) %>% roc_curve(Label, .pred_0)
log_reg_roc %>% ggplot() + geom_point(aes(1-specificity, sensitivity)) + labs(title = "AUC = 0.9533")
log_auc <- log_reg_res %>% augment(new_data = roc_train_data) %>% roc_auc(Label, .pred_0)
log_auc
...
xgboost_model = boost_tree(tree_depth = tune(), trees = 300, learn_rate = tune(), min_n = tune(), loss_reduction = tune(), sample_size = tune(), stop_iter = tune())%>%set_engine('xgboost') %>%set_mode('classification')
```
...
rf_model = rand_forest(mode='classification', mtry=4, trees=300, min_n = 20) %>% set_engine("ranger", num.threads=12)

NB_model = naive_Bayes(smoothness = 1.48, Laplace = 1.08)%>%set_mode('classification')

xgboost_model = boost_tree(tree_depth = 9	, trees = 300, learn_rate = 0.0408, min_n = 8, loss_reduction =0.0191,
                           sample_size = 0.944, stop_iter = 17)%>%set_engine('xgboost') %>%set_mode('classification')

# --------- K means Split ----------
feature_names = train_data %>% dplyr::select(-c(X,Y,Label, image, block)) %>% colnames()
formula = as.formula(paste0('Label ~ ', paste(feature_names, collapse = ' + '))) 

kmean_lr_roc_perf = workflow() %>% add_recipe(recipe(formula, data = train_data) %>% step_normalize(all_numeric_predictors())) %>%
  add_model(lr_model)%>%
  fit(data = train_data) %>%
  augment(new_data = kmeans_split$test)
kmean_lr_roc_test =  kmean_lr_roc_perf %>%select(Label, starts_with(".pred")) %>%
  yardstick::roc_curve(factor(Label),.pred_Cloudfree) %>%
...
kmean_lda_roc_perf = workflow() %>% add_recipe(recipe(formula, data = train_data) %>% step_normalize(all_numeric_predictors())) %>% add_model(lda_model)%>%
  fit(data = train_data) %>% augment(new_data = kmeans_split$test)
