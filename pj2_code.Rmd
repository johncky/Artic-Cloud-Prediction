...
CVmaster = function(tidymodel, train_features, training_labels, K, loss_functions, split_func,
  ...
  feature_names = train_features %>% select(-excld_cols) %>% colnames()
  ...
  output = data.frame()
  for (i in 1:K){
    output = rbind(output, res$.metrics[[i]])
  }
  N_loss =  output %>% distinct(.metric)%>%pull()%>%length()
  output = output %>% select(-c(.config, .estimator)) %>%
    mutate(fold = rep(1:K, each=N_loss)) %>%
    pivot_wider(names_from = .metric, values_from = .estimate)
  output
}

model = lr_model = logistic_reg() %>% set_engine('glm')
res = CVmaster(model, train_features = kmeans_split$train, training_labels = kmeans_split$train$Label, K = 5,
         split_func = Kmeans_block_split, loss_functions = metric_set(accuracy, kap))
