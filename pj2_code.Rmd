...
create_kmeans_split = function(data){
  ...
  return(list(train = data %>% 
         filter(image!=val_img_block$image & block!=val_img_block$block) %>%
         filter(image!=test_img_block$image & block!=test_img_block$block),
...
kmeans_split = create_kmeans_split(full_img)
block_split = create_block_split(full_img)
```

## Baseline
```{r}
kmeans_valid_accuracy <- mean((kmeans_split$val %>% filter(Label!=0) %>% pull(Label) )== -1)
kmeans_test_accuracy <- mean((kmeans_split$test %>% filter(Label!=0) %>% pull(Label) )== -1)

block_valid_accuracy <- mean((block_split$val %>% filter(Label!=0) %>% pull(Label) )== -1)
block_test_accuracy <- mean((block_split$test %>% filter(Label!=0) %>% pull(Label) )== -1)
