...
KS_stat = function(data){
  ...
  var_names = data %>% select(-rmv_cols) %>% colnames() 
  ...
  for (var in var_names){
    cloudy_density = data_cloudy %>% select(var) %>% as.matrix()%>% density()
    not_cloudy_density = data_not_cloudy %>% select(var)%>% as.matrix()%>% density()
    
    min_x = min(cloudy_density$x , not_cloudy_density$x)
    max_x = max(cloudy_density$x , not_cloudy_density$x)
    
    cloudy_density = data_cloudy %>% select(var) %>% as.matrix()%>% 
      density(from=min_x, to=max_x)
    
    not_cloudy_density = data_not_cloudy %>% select(var)%>% as.matrix()%>%
      density(from=min_x, to=max_x)
...
## CVmaster
```{r}
CVmaster = function(tidymodel, train_features, training_labels, K, loss_functions, split_func,
                    .scale=T){
  
  # Columns to exclude: X, Y, Label, image. These are not features
  excld_cols = train_features %>% colnames() %>% intersect(c('X', 'Y', 'image', 'Label'))
  ...
  split_idx = split_func(train_features, K)
  
  full_train = train_features %>% mutate(Label=training_labels)
  ...
  res = workflow() %>%
    ...
    add_model(tidymodel) %>%
    fit_resamples(resamples = resample_settings, metrics = loss_functions)
  ...
  #fold_loss = fold_loss %>% select(-c(.config, .estimator)) %>%
  fold_loss = fold_loss %>% dplyr::select(-c(.config, .estimator)) %>%
    mutate(fold = rep(1:K, each=N_loss)) %>%
    pivot_wider(names_from = .metric, values_from = .estimate)
  
  output = list(fold_loss = fold_loss, avg = (fold_loss %>%  dplyr::select(-fold) %>% colMeans()))
}

lr_model = logistic_reg() %>% set_engine('glm')
lda_model = discrim_linear() %>% set_engine('MASS')
lr_res = CVmaster(lr_model, train_features = kmeans_split$train, training_labels = kmeans_split$train$Label, K = 9,
         split_func = block_split, loss_functions = metric_set(accuracy, kap))
lda_res = CVmaster(lda_model, train_features = kmeans_split$train, training_labels = kmeans_split$train$Label, K = 9,
         split_func = block_split, loss_functions = metric_set(accuracy, kap))
