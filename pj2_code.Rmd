...
full_img_copy = rbind(full_img)
res = Kmeans_block_split(full_img, 12)
full_img_copy['block'] = NA

for (i in 1:12){
  full_img_copy[res[[i]],'block'] = i
}


full_img_copy %>% filter(image==1) %>% 
  ggplot() +
  geom_raster(aes(x=X, y=Y, fill=block)) +
  labs(title='Image 1 Label', x='X-coordinate', y='Y-coordinate')+
   theme(plot.title = element_text(hjust = 0.5), legend.position="none")
```
