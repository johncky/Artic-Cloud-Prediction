...
```{r}
# full_img %>% select(NDAI, SD, CORR) %>% pairs(lower.panel = NULL)
#full_img %>% select(contains('Rad')) %>% ggpairs(upper=NULL)
# full_img %>%  ggpairs()
```
...
p4 = plot_data%>%
  ggplot() + geom_density(aes(x=Rad_Df, fill=Label), alpha=0.5) 
