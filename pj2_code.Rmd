...
CVmaster = function(tidymodel, train_features, training_labels, K, metrics, split_func,
  ...
  if (!is.null(.tune_grid)){
    fold_loss = fold_loss %>% dplyr::filter(.config %in% (res%>%select_best()%>%pull(.config)))
  }
...
lr_reg_res = CVmaster(lr_reg_model,
                  train_features = train_data,
                  training_labels = train_data$Label,
                  K = 9,
                  split_func = Kmeans_block_split,
                  metrics = metric_set(accuracy, kap),
                  .tune_grid = 40)
write_rds(lr_reg_res, file='data/Kmeans_lr_reg_res.rds')
...
rf_res = CVmaster(rf_model,
                  ...
                  K = 9,
                  split_func = Kmeans_block_split,
                  metrics = metric_set(accuracy, kap), .tune_grid = 30)
...
lr_reg_res = CVmaster(lr_reg_model,
                  ...
                  metrics = metric_set(accuracy, kap),
                  .tune_grid = 40)
write_rds(lr_reg_res, file='data/Block_lr_reg_res.rds')
...
log_roc <- log_res %>% augment(new_data = roc_train_data) %>% roc_curve(Label, .pred_0)
