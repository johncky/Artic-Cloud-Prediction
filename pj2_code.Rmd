...
bk_split = create_block_split(full_img)
```
...
tbl_data %>%kbl(
  caption = "<center><strong>Data Split Ratio<center><strong>",
  escape = FALSE,
  format = 'html',
  table.attr = "style='width:45%;'") %>%
  kable_classic(full_width = T, html_font = "Cambria",) 
...
block_valid_accuracy <- mean((bk_split$val %>% filter(Label!=0) %>% pull(Label) )== -1)
block_test_accuracy <- mean((bk_split$test %>% filter(Label!=0) %>% pull(Label) )== -1)
