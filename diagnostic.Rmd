...
```{r}
bk_rf_pred %>% mutate(Label = factor(ifelse(Label==1, 'Cloudy', 'Cloud-free')),
...
error_data %>% pivot_longer(cols = c('.pred_-1', '.pred_1')) %>% ggplot()+
...
```{r}
error_data %>% mutate_at(4:11, funs(c(scale(.))))  %>% filter(Type %in% c('True Positive', 'False Positive'))%>%
...
```{r}
error_data %>% mutate_at(4:11, funs(c(scale(.)))) %>% filter(Type %in% c('True Negative', 'False Negative')) %>%
  pivot_longer(4:11)%>% ggplot()+
  geom_boxplot(aes(x = name, y=value, fill=Type), outlier.size = 0.1, outlier.alpha = 0.5) + 
  labs(title='Test Set Feature Range By Type')+theme(plot.title = element_text(hjust = 0.5))
```
...
tmp = error_data %>% pivot_longer(Rad_Df:Rad_An) %>% mutate(index = ifelse(name=='Rad_Af', 1, 
                             ifelse(name=='Rad_An', 2,
                                    ifelse(name=='Rad_Bf',3,
                                           ifelse(name=='Rad_Cf',4,5)))))%>% group_by(X, Y) %>% mutate(slope = lm(value ~ poly(index,2) )%>% 
                                                                                                         .$coefficients %>% .[3] %>% as.numeric())

tmp%>% 
  ggplot()+
  geom_density(aes(slope, fill=Type), alpha=0.5)
```
...
new_train_data_bk = rbind(bk_split$train, bk_split$val) %>% mutate(Label = factor(Label)) %>% pivot_longer(Rad_Df:Rad_An) %>% mutate(index = ifelse(name=='Rad_Af', 1, 
  ...
  mutate(slope = lm(value ~ poly(index,2) )%>% .$coefficients %>% .[3] %>% as.numeric()) %>% select(-index) %>% pivot_wider(names_from = name, values_from = value)%>% ungroup()
