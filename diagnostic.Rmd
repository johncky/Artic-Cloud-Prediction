...
p1 + p2 + plot_layout(nrow=2)+ plot_annotation(title='Convergence: Random Forest Test Performance',
                                                            theme = theme(plot.title = element_text(hjust = 0.5)))
```
### Stability
```{r}
...
plot_misclass = function(test_data, misclass_points){
  ...
  for (img in 1:3){
    ...
    if (dim(data)[1] > 0){
      ...
      p = ggplot() +
        ...
        geom_point(mapping=aes(x=misclass_points$X, y=misclass_points$Y), color='red')+
        scale_fill_manual(values=my.cols)+
        theme(plot.title = element_text(hjust = 0.5),  legend.position="none")+
        labs(title=paste0('Block Split: Test Set (Image ', img,') Misclassified Points'), x='X-coordinate', y='Y-coordinate')
...
misclass_points = bk_rf_pred %>% mutate(misclass = (Label != .pred_class)) %>% filter(misclass==T)
plot_misclass(test_data_bk, misclass_points)
```

```{r}
p1 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(NDAI, fill=Misclassified), alpha=0.5)

p2 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(CORR, fill=Misclassified), alpha=0.5)

p3 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(SD, fill=Misclassified), alpha=0.5)

p4 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(Rad_Df, fill=Misclassified), alpha=0.5)

p5 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(Rad_Cf, fill=Misclassified), alpha=0.5)

p6 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(Rad_Bf, fill=Misclassified), alpha=0.5)

p7 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(Rad_Af, fill=Misclassified), alpha=0.5)

p8 = bk_rf_pred %>% mutate(Misclassified = (Label != .pred_class)) %>% 
  ggplot()+
  geom_density(aes(Rad_An, fill=Misclassified), alpha=0.5)


p1+p2+p3+p4+p5+p6+p7+p8 + plot_layout(nrow=3,  guides = "collect")
```
