...
kmeans_cv_res %>% round(3)%>% summarise(Fold = c(Fold, 'Avg'),across(where(is.numeric), ~ c(., round(mean(.),3)))) %>%
  summarise(Fold = c(Fold, 'Std'),across(where(is.numeric), ~ c(., round(sqrt(var(.)),3)))) %>%
...
```{r}
block_cv_res %>% round(3)%>% summarise(Fold = c(Fold, 'Avg'),across(where(is.numeric), ~ c(., round(mean(.),3)))) %>%
  summarise(Fold = c(Fold, 'Std'),across(where(is.numeric), ~ c(., round(sqrt(var(.)),3)))) %>%
  relocate(LDA, .before = QDA)%>%
